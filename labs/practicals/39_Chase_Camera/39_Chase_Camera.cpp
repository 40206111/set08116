...
bool update(float delta_time) {
	...
	{
		target_mesh.get_transform().rotate(vec3(0.0f, 2.0f * delta_time, 0.0f));
	}
	...
	{
		target_mesh.get_transform().translate(vec3(0.0f, 0.0f, -10.0f * delta_time));
	}
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_A))
	{
		target_mesh.get_transform().translate(vec3(-10.0f * delta_time, 0.0f, 0.0f));
	}
	...
	vec3 to_cube = cam.get_target_pos() - cam.get_position();
	to_cube.y = 0.0f;
	vec3 to_cube_n = normalize(to_cube);
	vec3 perp_cube = to_cube_n;
	mat3 perping = rotate(mat4(1.0f), pi<float>()/2, vec3(0.0f,1.0f,0.0f));

	perp_cube = perping * perp_cube;

	vec3 movement = vec3(0.0f, 0.0f, 0.0f);

	cout << to_cube_n.x << ":" << to_cube_n.z << "," << perp_cube.x << ":" << perp_cube.z << endl;
	...
	vec3 movement_n = vec3(0.0f);
	if (movement != vec3(0.0f)) {
		movement_n = normalize(movement);
		movement_n *= speed;
	}
	target_mesh.get_transform().translate(movement_n);

...
bool load_content() {
  ...
  // Create plane mesh
	...
	outline.add_shader("27_Texturing_Shader/simple_texture.vert", GL_VERTEX_SHADER);
	outline.add_shader("shaders/mask.frag", GL_FRAGMENT_SHADER);
	simple.add_shader("27_Texturing_Shader/simple_texture.vert", GL_VERTEX_SHADER);
	simple.add_shader("27_Texturing_Shader/simple_texture.frag", GL_FRAGMENT_SHADER);
...
bool render() {
	...
	for (auto &e : meshes) {
		...
		glUniformMatrix4fv(simple.get_uniform_location("MVP"), 1, GL_FALSE, value_ptr(MVP));
		// Bind texture
		renderer::bind(tex, 0);
		// Set tex uniform
	...
	mat4 MVP(1.0f);
	// Set MVP matrix uniform
	glUniformMatrix4fv(outline.get_uniform_location("MVP"), 1, GL_FALSE, value_ptr(MVP));
	// Bind texture
	renderer::bind(frame.get_frame(), 0);
	// Set tex uniform
	glUniform1i(outline.get_uniform_location("tex"), 0);
	// Set inverse width Uniform
	glUniform1f(outline.get_uniform_location("inverse_width"), 1.0f / renderer::get_screen_width());
	// Set inverse height Uniform
	glUniform1f(outline.get_uniform_location("inverse_height"), 1.0f / renderer::get_screen_height());
	...
	renderer::bind(mask.get_frame(), 2);
	glUniform1i(tex_eff.get_uniform_location("mask"), 2);

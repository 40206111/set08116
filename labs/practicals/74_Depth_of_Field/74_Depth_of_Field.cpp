...
bool render() {
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		// Set inverse width
		glUniform1f(blur.get_uniform_location("inverse_width"), 1 / renderer::get_screen_width());
		// Set inverse height
		glUniform1f(blur.get_uniform_location("inverse_height"), 1 / renderer::get_screen_height());
		// Render screen quad

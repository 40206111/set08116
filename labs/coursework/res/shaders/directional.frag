...
vec4 calculate_direction(in directional_light light, in material mat,
  ...
  float shade_tex_coord =
      clamp(max(dot(transformed_normal, light.light_dir), 0.0), 0.0, 1.0);
  // calculate diffuse
  vec4 diffuse = texture(shade, shade_tex_coord) *
                 (mat.diffuse_reflection * light.light_colour);
  ...
  // calculate specular
  vec4 specular = texture(shade, shade_tex_coord) *
                  (mat.specular_reflection * light.light_colour);
  // caluclate colour

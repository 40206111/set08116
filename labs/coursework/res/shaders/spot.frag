...
vec4 calculate_spot(in spot_light spot, in material mat, in vec3 vertex_pos,
  ...
  float shade_tex_coord =
      clamp(((pow(intensity, spot.power)) / att_fat), 0.0, 1.0);
  // calculate light colour
  vec4 light_col = texture(shade, shade_tex_coord) * spot.light_colour;

  // calculate shade texture coords for diffuse
  shade_tex_coord =
      clamp(max(dot(transformed_normal, light_dir), 0.0), 0.0, 1.0);
  // calculate diffuse
  vec4 diffuse =
      texture(shade, shade_tex_coord) * (mat.diffuse_reflection * light_col);
  // calculate half vector
  vec3 half_v = normalize(light_dir + view_dir);
  // calculate shade texture coords for specular
  shade_tex_coord = clamp(
      pow(max(dot(transformed_normal, half_v), 0.0), mat.shininess), 0.0, 1.0);
  // calculate specular
  vec4 specular =
      texture(shade, shade_tex_coord) * (mat.specular_reflection * light_col);
  // caluclate colour
